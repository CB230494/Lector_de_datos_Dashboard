...
# - Imports
# - Bandera de features
# - Normalización / parsers robustos
# - Lectura/búsqueda en Excel (layout-independiente)
# - Helpers para columnas dinámicas y tokenización
...
# -----------------------------------------------------------
# Bandera para ocultar/mostrar el panel extra de filtros UI
# -----------------------------------------------------------
# Si lo quieres oculto (como pediste): False
# Si quieres volver a verlo más adelante: True
...
# -----------------------------------------------------------
def _norm(x: str) -> str:
    """ASCII, colapsa espacios, minúsculas. Para comparar contenidos."""
    if x is None:
        return ""
    x = str(x)
...
def _norm_str(s: str) -> str:
    """Versión para nombres de columnas/etiquetas (conserva palabras)."""
    if s is None:
        return ""
    s = str(s)
...
def _to_int(x) -> Optional[int]:
    """
    Entero solo si parece cantidad corta (1–3 dígitos).
    Evita años (p.ej. 2025) y celdas con '%' dentro.
    """
    if x is None:
        return None
    s = str(x).strip()
...
def _to_num_safe(x, pct: bool=False) -> float:
    """
    Conversión resiliente a float.
    pct=True: quita '%' y devuelve 0..100 si procede.
    Si falla, toma el primer número que aparezca en el string.
    """
    if pd.isna(x):
        return 0.0
    s = str(x).strip()
...
# -----------------------------------------------------------
# Lectura y búsqueda en matrices Excel (layout-independiente)
# -----------------------------------------------------------
def _read_df(file) -> pd.DataFrame:
    """Lee Excel sin encabezados, todo como texto (robusto a layouts variados)."""
    return pd.read_excel(file, engine="openpyxl", header=None, dtype=str)

def _find(df: pd.DataFrame, pattern: str) -> List[Tuple[int, int]]:
    """Encuentra celdas cuyo contenido (normalizado) coincide con regex 'pattern'."""
...
def _neighbors(df: pd.DataFrame, r: int, c: int, up: int, down: int, left: int, right: int):
    """Itera coordenadas en la vecindad de (r,c) con límites seguros."""
    r0 = max(0, r - up)
...
def _pick_best_count(cands: List[int], max_allowed: int = 60) -> Optional[int]:
    """Escoge el mejor conteo plausible (0..max_allowed) de una lista."""
...
def _pick_named_column(df: pd.DataFrame, names_like: List[str]) -> Optional[str]:
    """
    Intenta mapear alias -> nombre real de columna (tolerante a tildes/espacios).
    Ej.: ['categorias','categoría','categoria'] -> 'Categorías'
    """
...
# ---- Detectar delegación en las primeras filas o por rótulo ----
def detect_delegacion(df: pd.DataFrame) -> Optional[str]:
    """Detecta texto tipo 'D58 - Sarapiquí' o celdas junto a 'Delegación'."""
...
# ---- Detección de cantidad de líneas de acción ----
def detect_lineas_accion(df: pd.DataFrame, debug: bool=False) -> Optional[int]:
    """Busca rótulos 'Líneas de Acción' y extrae número cercano."""
...
# ---- Filas de indicadores (GL/FP) ----
def detect_indicator_rows(df: pd.DataFrame) -> List[int]:
    """Devuelve filas que contienen GL o FP a la izquierda."""
    rows = []
    for r in range(df.shape[0]):
...
def detect_avance_columns(df: pd.DataFrame) -> List[int]:
    """Detecta columnas con rótulos 'Avance'."""
    cols = []
    for r in range(df.shape[0]):
        ...
        if len(cols) >= 2:
            ...
            cols = []
    # fallback genérico si no los detecta
...
# ---- Estado de fila a partir de celdas de avance ----
...
# ---- Detectar total de indicadores explícito ----
...
# ================================
# Lee el Excel, detecta metadatos y arma una fila de salida
# con avance global + por rol (GL/FP).
...
def process_file(upload, debug: bool = False) -> Dict:
    ...
    # 3) Avance global (todas las filas GL/FP)
    ...
    # 7) Salida estándar para el consolidado
    out = {
        "archivo": upload.name,
        "delegacion": deleg,
...
def _resumen_avance(col, sin_n, sin_p, con_n, con_p, comp_n, comp_p, total_ind):
    """Bloque 'Avance de Indicadores' con tarjetas y total."""
    with col:
        st.markdown(f"""
        <div style="background:{COLOR_AZUL_H};color:white;padding:10px 12px;border-radius:8px 8px 0 0;
...
def _ensure_numeric(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convierte a numérico las columnas clave (cuentas y porcentajes).
...
def _ensure_numeric(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df
# ==================================================
# PARTE 7/10 — LÍNEAS DE ACCIÓN + FILTROS EXTRA UI
# ==================================================

# -------- Totales y desglose de Líneas de Acción ----------
def _lineas_tot_y_desglose(agg: pd.Series):
    """Retorna (total, gl, fp, mx, tiene_desglose:bool)."""
...
def _render_lineas_block(agg: pd.Series):
    """Render visual limpio de Líneas de Acción."""
    total, gl, fp, mx, has_breakdown = _lineas_tot_y_desglose(agg)
    _big_number(total, "Líneas de Acción", big_px=72)
    if has_breakdown:
        c1, c2, c3 = st.columns(3)
        c1.markdown(f"""
            <div style="background:#ffffff;border:1px solid #e3e3e3;border-radius:8px;
...
# -------- Filtros adicionales (categoría / problemática / responsable) ----------
def _pick_named_column(df: pd.DataFrame, names_like: List[str]) -> Optional[str]:
    if df is None or df.empty:
        return None
    norm_map = {_norm_str(c): c for c in df.columns}
...
def _extract_tokens(cell) -> List[str]:
    """Separa celdas con múltiples valores (coma, punto y coma, saltos de línea, etc.)."""
    if pd.isna(cell):
        return []
    s = str(cell)
    parts = re.split(r"[,\;/\|\n]+", s)
...
def _role_flags(row: pd.Series) -> Tuple[bool, bool, bool]:
    """
    Determina si una fila está marcada para GL / FP / Mixta.
    Usa columnas de Líneas o Indicadores si existen.
    """
...
def _sum_lineas(df_any: pd.DataFrame) -> Tuple[int, int, int, int]:
    """Devuelve (total, gl, fp, mx) de líneas de acción si existen las columnas."""
...
def _extra_filters_ui(scope_df: pd.DataFrame, key_prefix: str = "") -> Tuple[pd.DataFrame, Dict[str, str]]:
    """UI de filtros: Categoría / Problemática / Responsable."""
    ...
    if not found:
        st.caption("No se encontraron **Categorías**, **Problemática** o **Responsable** en el Excel.")
        return scope_df, {}
    ...
    if col_categorias:
        ...
        sel = c1.multiselect("Categorías", options=opts, default=opts, key=f"{key_prefix}cat")
    ...
    return df_f, found
# ==========================================================
# PARTE 8/10 — RESUMEN TEXTO LIMPIO (SOLO ATENDIDOS)
# ==========================================================
# Muestra debajo del dashboard algo como:
#   Se atienden N líneas de acción.
#   Riesgo Social
#     • Consumo de alcohol en vía pública — atendido por: Gobierno Local
#     • Personas en situación de calle — atendido por: Gobierno Local, Fuerza Pública, Mixta
#   Delito
#     • Venta de drogas — atendido por: Fuerza Pública
#
# Reglas:
# - NO mostrar "(GL:x, FP:y, Mixtas:z)" en el título.
...
def _render_resumen_categorias_limpio(df_filt: pd.DataFrame, found_cols: Dict[str, str]):
    ...
    # 2) Construcción de mapa: categoría -> [bullets] SOLO si hay atención
    cat_map: Dict[str, List[str]] = {}

    for _, row in df_filt.iterrows():
        gl_f, fp_f, mx_f = _role_flags(row)
        if not (gl_f or fp_f or mx_f):
            continue  # omitimos sin atención
    ...
    # 3) Ordenar categorías (Riesgo(s) → Delito(s) → alfabético)
    ...
    for cat in sorted(cat_map.keys(), key=_cat_order_key):
        bullets = [b for b in cat_map[cat] if b]  # limpieza
        if not bullets:
            continue
        printed_any = True
        st.markdown(f"**{cat}**")
...
# =======================================================
# PARTE 9/10 — DASHBOARD + RESUMEN LIMPIO INTEGRADO
# =======================================================
...
if dash_file:
    # 1) Cargar hoja 'resumen' (si no existe, usa la primera)
    try:
    ...
    # 3) DR_inferida a partir de columna detectada o de 'Delegación'
    ...
    with tabs[0]:
        ...
        if "Delegación" not in df_dash.columns:
            ...
            # -------- Resumen por categorías (solo atendidos) --------
            if SHOW_EXTRA_FILTERS:
                df_catprob, found_cols = _extra_filters_ui(scope_df, key_prefix="deleg_")
            else:
                # Sin panel: usar el df tal cual y detectar columnas automáticamente
                df_catprob = scope_df
                found_cols = {
                    k: v for k, v in {
